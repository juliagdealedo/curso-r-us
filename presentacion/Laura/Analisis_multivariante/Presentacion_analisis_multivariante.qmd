...
#| echo: true
library(here)
clima <- read.table(here("data/exoticas.txt", header = T, sep = "\t"))
str(clima)
```

[Alien : variable respuesta]{style="font-size:25px"}
...
[Representación de las distancias existentes entre un conjunto de datos en un espacio geométrico de pocas dimensiones.]{style="font-size:30px"}

[1. Cálculo matriz de disimilaridad *X* a partir de la matriz de datos.]{style="font-size:25px"}

[2. Asignación de unidades muestrales a una configuración inicial aleatoria]{style="font-size:25px"}
...
[1. Explorar visualmente cómo son de similares las parcelas muestreadas en función de la composición de especies]{style="font-size:30px"}

[2. Investigar relación de esta ordenación y las variables ambientales.]{style="font-size:30px"}
...
[Gráfico con diferencias por tipo de bosque y relación lineal con variables]{style="font-size:30px"}
